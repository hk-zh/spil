...
from spil.evaluation.multistep_sequences import get_sequences, get_sequences_fix
from spil.evaluation.utils import get_default_model_and_env, get_env_state_for_initial_condition, join_vis_lang
...
def count_success(results):
    ...
    step_success = []
    for i in range(1, 5):
        n_success = sum(count[j] for j in reversed(range(i, 5)))
...
def evaluate_policy(model, env, lang_embeddings, args):
    ...
    eval_sequences = get_sequences_fix(args.num_sequences)
    ...
    for initial_state, eval_sequence in eval_sequences:
        ...
        if not args.debug:
            eval_sequences.set_description(
                " ".join([f"{i + 1}/4 : {v * 100:.1f}% |" for i, v in enumerate(count_success(results))]) + "|"
            )
